...
### Fixed
- **`set-clipboard` narrowed to `external`** (`tmux/tmux.conf`): tmux still sends the clipboard out to the terminal, so yanking over SSH is unaffected, but no longer lets a program in a pane set tmux buffers with an OSC 52 escape. Neovim yanks were verified to reach the tmux buffer under both settings.

  An earlier version of this entry credited the change with fixing stray text appearing in new shells. That was wrong: the stray text was a bare Postgres connection string on line 125 of the user's `~/.bashrc`, which every new shell tried to execute. Nothing in this repo caused it.
