...
### Fixed
- **Stray text no longer gets typed into new shells** (`tmux/tmux.conf`): `set-clipboard` was set to `on`, which makes tmux answer OSC 52 clipboard *queries* from programs in a pane. The answer arrives as keyboard input, so a clipboard read by Neovim (`clipboard=unnamedplus` with no X display) could land a yanked value at a shell prompt and get executed (`bash: <text>: command not found`). Now `external`, which still sends the clipboard out to the terminal — the SSH yank fix is unaffected — but ignores inbound clipboard sequences.
